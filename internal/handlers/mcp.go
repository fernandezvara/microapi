...
func listSetsMCP(db *sql.DB, w http.ResponseWriter) {
	...
	if err != nil { middleware.WriteJSON(w, http.StatusInternalServerError, false, nil, models.Ptr(err.Error())); return }
	defer rows.Close()
	var sets []string
	for rows.Next() { var s string; _ = rows.Scan(&s); sets = append(sets, s) }
...
func createDocMCP(h *Handlers, w http.ResponseWriter, args map[string]any) {
	...
	if set == "" || collection == "" { middleware.WriteJSON(w, http.StatusBadRequest, false, nil, models.Ptr("set and collection are required")); return }
...
func getDocMCP(h *Handlers, w http.ResponseWriter, args map[string]any) {
	...
	collection, _ := args["collection"].(string)
	id, _ := args["id"].(string)
	if set == "" || collection == "" || id == "" { middleware.WriteJSON(w, http.StatusBadRequest, false, nil, models.Ptr("set, collection and id are required")); return }
	if err := middleware.ValidateNames(set, collection); err != nil { writeErr(w, err); return }
	if err := database.EnsureSetTable(h.db, set); err != nil { writeErr(w, err); return }
...
func queryCollectionMCP(h *Handlers, w http.ResponseWriter, args map[string]any) {
	...
	if v, ok := args["offset"].(float64); ok { offset = int(v) }
	includeMeta := true
	if v, ok := args["include_meta"].(bool); ok { includeMeta = v }
	if set == "" || collection == "" { middleware.WriteJSON(w, http.StatusBadRequest, false, nil, models.Ptr("set and collection are required")); return }
	if err := middleware.ValidateNames(set, collection); err != nil { writeErr(w, err); return }
	if err := database.EnsureSetTable(h.db, set); err != nil { writeErr(w, err); return }
